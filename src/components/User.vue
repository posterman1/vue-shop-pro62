    ...
    <el-card class="box-card">
      ...
      <el-table :data="userInfos" border style="width: 100%">
        ...
        <el-table-column prop="mg_state" label="状态" width="70">
          ...
          <!--slot-scope="info"代表的是el-switch组件接收插槽传递过来的数据--->
          <!---v-model="info.row.mg_state"代表双向数据绑定用户状态---->
          <!---给el-switch绑定事件，事件名称为changeState并且传递了参数id和mg_state--->
          <el-switch
...
export default {
  ...
  methods: {
    ...
    async showEditDialog(id) {
      ...
      // 获取被修改用户id成功 把获取的数据赋予给form表单对象
      this.editForm = res.data
      // 现在的editForm样子为
      // editFrom:{username:xxx,email:xxxx,mobile:xxxx,id:xxx,role_id:xxxx}
      // id:当前被修改用户的id 后期可以通过this.editFrom.id获取正在修改用户的id
    },
    // 修改用户相关2
    // 删除用户
    delUser(id) {
    ...
    async getUserInfos() {
      ...
      // axios发送请求的格式为：http://主机名:端口/xxx/users?query=&pagenum=1&pagesize=3
      ...
      // 把获得好的用户数据赋予userInfos成员
      this.userInfos = res.data.users
    }
  },
  data() {
    // 为校验手机号码生成一个函数
    // var checkMobile=(rule,value被校验的数据,callback回调方法)=>{}
    var checkMobile = (rule, value, callback) => {
      // 手机号校验规则：1开始后边接3、5.8.7.9 后边三位数
      // 正则表达式
